...
interface PolicyDetailsProps {
  isModal?: boolean; // Optional flag to adjust behavior/styling for modal context
  closeMethod?: any
}
...
export default function PolicyDetails({
  ...
  const handleDomainClick = (domain: DomainId) => {
    if (isModal) {
      ...
      router.back();
      // Use setTimeout to ensure the navigation happens after the modal closes
      setTimeout(() => {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-sm p-6 relative" >
      {/* Back button */}
      <button
        onClick={closeMethod ? closeMethod : () => { router.push('/') }}
